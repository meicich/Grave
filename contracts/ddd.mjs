...
// デプロイされたコントラクトのアドレス
const contractAddress = '0xB48C28F908a7B3b7a7832DD2e9C8F2710B80A962';
...
async function mintNFT() {
  const bitmap = await fs.readFile('test.png');

  // バイナリデータを base64 に変換
  const base64Image = new Buffer.from(bitmap).toString('base64');
  console.log(base64Image.length/1000)
  console.log(base64Image.length/10000)
...
// NFTのデータを収集する関数
async function collectData(tokenId) {
  const bitmap = await fs.readFile('test.png');
...
async function main() {
  await mintNFT();
  await collectData(292); // tokenIdは既に発行されたものを使用してください
}
